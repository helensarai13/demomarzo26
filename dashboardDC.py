...
st.subheader('Tendencia de Personajes por Año de Primera Aparición (Selección de Rango)')
selected_years = st.slider(
    'Selecciona el rango de años para la tendencia:',
    min_year,
    max_year,
    (min_year, max_year)
)
...
filtered_year_counts = year_counts[
    (year_counts['Year'] >= selected_years[0]) & (year_counts['Year'] <= selected_years[1])
]
...
fig_year_line = px.line(
    ...
    x='Year',
    y='Count',
    title=f'Tendencia de Personajes por Año de Primera Aparición ({selected_years[0]}-{selected_years[1]})',
